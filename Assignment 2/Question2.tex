...
\begin{solution}
    T : Trial Number at which first heads appears
    \[P(T = t) = (1 - p)^{t - 1} p\]
    For n independent coin tosses,
    Let $P(T = i)  = T_i$ be the trial that first heads comes on the i$^{th}$ coin.
    To find expectation of X, we use 
    \[E[T] = \sum_{i=1}^n i * P( T_i) \]
    \[E[T] = \sum_{i=1}^ni(1 - p)^{i - 1} p\]
    This is an arithmetico-geometric series.
    Multiplying by 1-p,
    \[(1 - p)E[T] = \sum_{i=1}^n i(1 - p)^i p\]
    Subtracting the two equations,
    \[pE[T] = p(1 + (1-p) + (1-p)^2 + ... + (1-p)^{(n-1)} - n(1-p)^n)\]
